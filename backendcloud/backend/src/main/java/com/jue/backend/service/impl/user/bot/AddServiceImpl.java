...
import com.jue.backend.service.user.bot.AddService;
...
public class AddServiceImpl implements AddService {
    ...
    public Map<String, String> add(Map<String, String> data) {
        // 获取SecurityContextHolder容器中当前用户的身份认证信息（登录时自动绑定），返回Authentication实例
        UsernamePasswordAuthenticationToken authenticationToken =
                (UsernamePasswordAuthenticationToken) SecurityContextHolder.getContext().getAuthentication();
        // 强制转换为用户信息，UsernamePasswordAuthenticationToken转为UserDetailsImpl
        ...
        Date now = new Date();
        // id自增、id、title、description、content、rating1500
